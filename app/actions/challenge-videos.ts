...
export async function fetchAllChallenges(): Promise<Challenge[]> {
  ...
  try {
    console.log(`📊 Fetching today's challenges from Supabase (${today})...`)
    ...
    if (data && !error && data.length > 0) {
      console.log(`✅ Found ${data.length} challenges in Supabase for today`)      // Convert Supabase data to Challenge format
      return data.map(row => {
      ...
      console.log("⚠️ No challenges found in Supabase for today. Auto-triggering daily refresh...")
    ...
    try {
      ...
      if (refreshResult.success) {
        ...
        if (newData && !newError && newData.length > 0) {
          console.log(`✅ Found ${newData.length} challenges after auto-refresh`)
        ...
        console.error("❌ Auto-refresh failed:", refreshResult.message)
      }
    } catch (refreshError) {
      console.error("❌ Error during auto-refresh:", refreshError)
    }
    
    console.log("⚠️ No challenges available even after auto-refresh")
    return []
...
// Hàm lấy thử thách hiện tại cho người dùng (homepage)
export async function fetchCurrentChallenge(): Promise<Challenge | null> {
  try {
    const today = getTodayDate()
    
    console.log("🔍 Checking Supabase for today's current challenge")
    ...
    if (challenges.length > 0) {
      ...
      console.log("✅ Using first challenge as current challenge:", currentChallenge.id)
      return currentChallenge
    }

    console.log("⚠️ No current challenge found for today. Waiting for cron job to generate new challenges.")
    return null
...
export async function generateDailyChallenges(): Promise<Challenge[]> {
  ...
  try {
    console.log(`🚀 Generating 10 challenges for ${today}...`)
    ...
    console.log(`⚡ Generating all 10 challenges in parallel...`)
    ...
    const challengePromises = difficulties.map(async (difficulty, index) => {
      ...
      try {
        console.log(`📹 Starting ${difficultyType} challenge ${index + 1}/10...`)
        ...
        while (attempts < 2 && !videoData) { // Reduced from 3 to 2 attempts
          ...
          try {
            ...
            console.error(`❌ Attempt ${attempts} failed for ${difficultyType} challenge:`, error)
          }
        }
        
        if (!videoData) {
          console.error(`❌ Failed to get video for ${difficultyType} challenge after 2 attempts`)
          return null
        }
        ...
        if (!transcript || transcript === "Transcript unavailable" || transcript.length < 100) {
          try {
            console.log(`🤖 Extracting transcript for ${difficultyType} challenge: ${challenge.id}`)
...
Please transcribe the ENTIRE audio content word-for-word:`
              ...
              if (geminiTranscript && geminiTranscript.length > 50) {
                ...
                console.log(`✅ Gemini AI transcript extracted for ${challenge.id}: ${transcript.length} characters`)
              } else {
                console.log(`⚠️ Gemini AI returned short transcript for ${challenge.id}, using fallback`)
                transcript = `Educational content about ${challenge.title}. ${challenge.description}`
              }
            } else {
              console.log(`⚠️ No GEMINI_API_KEY, using fallback transcript for ${challenge.id}`)
              transcript = `Educational content about ${challenge.title}. ${challenge.description}`
            }
          } catch (error) {
            console.error(`❌ Gemini AI transcript failed for ${challenge.id}:`, error)
            transcript = `Educational content about ${challenge.title}. ${challenge.description}`
          }
        }
        ...
        console.log(`✅ Generated ${difficultyType} challenge: ${challenge.title} (${challenge.id})`)
        
        return challenge
        
      } catch (error) {
        console.error(`❌ Error generating ${difficultyType} challenge:`, error)
        return null
      }
    })
    
    // Wait for ALL challenges to complete in parallel
    console.log(`⏳ Waiting for all 10 challenges to complete...`)
    ...
    for (const challenge of validChallenges) {
      if (!usedVideoIds.has(challenge.id)) {
        ...
        console.log(`🔄 Skipping duplicate video: ${challenge.id}`)
      }
    }
    
    console.log(`🎯 Generated ${challenges.length} unique challenges out of ${validChallenges.length} total`)
    
    // If we have less than 10 unique challenges, add some fallback logic
    if (challenges.length < 10) {
      console.log(`⚠️ Only ${challenges.length} unique challenges generated, proceeding with what we have`)
    }
      // Save all challenges to Supabase
    if (challenges.length > 0) {
      console.log(`💾 Saving ${challenges.length} challenges to Supabase...`)
      ...
      if (error) {
        console.error("❌ Error saving challenges to Supabase:", error)
        throw error
      }
      
      console.log(`✅ Successfully saved ${challenges.length} challenges to Supabase`)
    }
