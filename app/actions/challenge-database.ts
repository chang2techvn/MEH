...
export async function saveChallengeToDatabase(challenge: Challenge, isAutoGenerated = true) {
  try {
    ...
    const challengeData = {
      ...
      dailyBatch: isAutoGenerated ? getTodayBatch() : null,      expiresAt: isAutoGenerated ? expiresAt.toISOString() : null,
      isCompleted: false
    }
...
export async function getTodaysChallengesFromDatabase(): Promise<Challenge[] | null> {
  try {
    ...
    const { data: autoGenerated, error: autoError } = await supabaseServer
      .from('challenges')
      .select('*')
    ...
    const { data: userCreated, error: userError } = await supabaseServer
      .from('challenges')
      .select('*')
...
export async function cleanupOldDailyChallenges() {  try {
    const { data, error } = await supabaseServer.rpc('cleanup_old_daily_challenges')
...
export async function cleanupExpiredChallenges() {  try {
    const { data, error } = await supabaseServer.rpc('cleanup_expired_challenges')
...
export async function markChallengeCompleted(challengeId: string, userId: string) {  try {
    const { data, error } = await supabaseServer
      .from('challenges')
      .update({
        isCompleted: true,
...
function convertDatabaseToChallenge(dbChallenge: any): Challenge {
  ...
  return {
    ...
    isAutoGenerated: dbChallenge.isAutoGenerated || false,
    dailyBatch: dbChallenge.dailyBatch || null
  }
}
