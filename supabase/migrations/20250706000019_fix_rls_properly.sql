...
-- SIMPLE POLICIES for conversation_participants table
-- Policy 1: Users can view their own participation records
CREATE POLICY "Users can view own participation" ON conversation_participants
  FOR SELECT 
  USING (user_id::text = auth.uid()::text);

-- Policy 2: Users can insert their own participation
CREATE POLICY "Users can insert own participation" ON conversation_participants
  FOR INSERT 
  WITH CHECK (user_id::text = auth.uid()::text);

-- Policy 3: Conversation creators can add participants
CREATE POLICY "Creators can add participants" ON conversation_participants
  FOR INSERT 
  WITH CHECK (
    EXISTS (
...
-- Enable RLS (keep security)
ALTER TABLE conversations ENABLE ROW LEVEL SECURITY;
ALTER TABLE conversation_participants ENABLE ROW LEVEL SECURITY;

-- Add comments
COMMENT ON TABLE conversations IS 'RLS enabled with simple non-recursive policies';
COMMENT ON TABLE conversation_participants IS 'RLS enabled with simple non-recursive policies';
