-- Fix UUID issues in approval functions

-- Drop and recreate approval functions with proper UUID handling
DROP FUNCTION IF EXISTS approve_user_account(UUID, UUID);
DROP FUNCTION IF EXISTS reject_user_account(UUID, UUID, TEXT);

-- Create approval function that handles NULL admin_id properly
...
-- Create rejection function that handles NULL admin_id properly
